...
func getGadgetImageDescriptor(ctx context.Context, store *oci.Store, fullTag string) (*GadgetImageDesc, error) {
	...
	index, err := getIndex(ctx, store, fullTag)
	if err != nil {
	...
	for i := 0; i < len(index.Manifests); i++ {
		if index.Manifests[i].Platform != nil {
			architectures = append(architectures, index.Manifests[i].Platform.Architecture)
		}
	}
	...
	// Extract ebpf params from config
	ebpfParams := []string{}
	...
	var ebpfLayer, wasmLayer ocispec.Descriptor
	for _, layer := range manifest.Layers {
		if layer.MediaType == eBPFObjectMediaType {
			ebpfLayer = layer
		} else if layer.MediaType == wasmObjectMediaType {
			wasmLayer = layer
		}
	}
